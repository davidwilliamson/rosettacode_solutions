...
def solved(pegs):
    """Return True if the current state of pegs is a solved state"""
...
def move_disk(from_peg, to_peg):
    """Move a disk from the from_peg to the to_peg"""
    disk = from_peg.pop(0)
    to_peg.insert(0, disk)


def top_disk(peg):
    """Return the disk on top of the provied peg"""
    return peg[0]


def legal_move(from_peg, to_peg):
    """return True if it is legal to move the top disk from the
    from_peg to the to_peg
    """
    if not from_peg:
        return False
    if not to_peg:
...
def move_between_pair_of_pegs(peg_1, peg_2):
    """Move the top disk from peg_1 to peg_1 if that is a legal move;
    Move it from peg_2 to peg_1 otherwise
    """
